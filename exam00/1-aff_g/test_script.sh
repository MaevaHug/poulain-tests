...
cc -Wall -Wextra -Werror -o aff_g aff_g.c
if [ $? -ne 0 ]; then
...
longest_test="$> ./aff_g \"zaz sent le poney\" | cat -e"
...
run_test() {
	...
	echo "$separator"
	echo "$> ./aff_g $args | cat -e"

	output=$(eval ./aff_g $args | cat -e)
...
# Test cases
run_test "\"abc\"" "g\$" || all_tests_passed=false
run_test "\"dubO a POIL\"" "g\$" || all_tests_passed=false
run_test "\"zaz sent le poney\"" "g\$" || all_tests_passed=false
run_test "" "g\$" || all_tests_passed=false
