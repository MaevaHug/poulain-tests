...
cc -Wall -Wextra -Werror -o repeat_alpha repeat_alpha.c
if [ $? -ne 0 ]; then
...
longest_test="$> ./repeat_alpha \"abacadaba 42!\" | cat -e"
...
run_test() {
	...
	echo "$separator"
	echo "$> ./repeat_alpha $args | cat -e"

	output=$(eval ./repeat_alpha $args | cat -e)
...
# Test cases
run_test "\"abc\"" "abbccc\$" || all_tests_passed=false
run_test "\"Alex.\"" "Alllllllllllleeeeexxxxxxxxxxxxxxxxxxxxxxxx.\$" || all_tests_passed=false
run_test "\"abacadaba 42!\"" "abbacccaddddabba 42!\$" || all_tests_passed=false
