...
cc -Wall -Wextra -Werror -o ft_rrange ft_rrange.c main.c
if [ $? -ne 0 ]; then
...
longest_test="$> ./ft_rrange -5 -3 | cat -e"
...
run_test() {
	...
	echo "$separator"
	echo "$> ./ft_rrange $args | cat -e"

	output=$(eval ./ft_rrange $args | cat -e)
...
# Test cases
run_test "1 3" "3 2 1\$" || all_tests_passed=false
run_test "-1 2" "2 1 0 -1\$" || all_tests_passed=false
run_test "0 0" "0\$" || all_tests_passed=false
run_test "0 -3" "-3 -2 -1 0\$" || all_tests_passed=false
run_test "5 5" "5\$" || all_tests_passed=false
run_test "10 7" "7 8 9 10\$" || all_tests_passed=false
run_test "-5 -3" "-3 -4 -5\$" || all_tests_passed=false
