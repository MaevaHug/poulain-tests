...
cc -Wall -Wextra -Werror -o union union.c
if [ $? -ne 0 ]; then
...
longest_test="$> ./union \"ddf6vewg64f\" \"gtwthgdwthdwfteewhrtag6h4ffdhsd\" | cat -e"
...
run_test() {
	...
	echo "$separator"
	echo "$> ./union $args | cat -e"

	output=$(eval ./union $args | cat -e)
...
# Test cases
run_test "\"zpadinton\" \"paqefwtdjetyiytjneytjoeyjnejeyj\"" "zpadintoqefwjy\$" || all_tests_passed=false
run_test "\"ddf6vewg64f\" \"gtwthgdwthdwfteewhrtag6h4ffdhsd\"" "df6vewg4thras\$" || all_tests_passed=false
run_test "\"rien\" \"cette phrase ne cache rien\"" "rienct phas\$" || all_tests_passed=false
