...
cc -Wall -Wextra -Werror -o aff_first_param aff_first_param.c
if [ $? -ne 0 ]; then
...
longest_test="$> ./aff_first_param vincent mit \"l'ane\" dans un pre et \"s'en\" vint | cat -e"
...
run_test() {
	...
	echo "$separator"
	echo "$> ./aff_first_param $args | cat -e"

	output=$(eval ./aff_first_param $args | cat -e)
...
# Test cases
run_test "vincent mit \"l'ane\" dans un pre et \"s'en\" vint" "vincent\$" || all_tests_passed=false
run_test "\"j'aime le fromage de chevre\"" "j'aime le fromage de chevre\$" || all_tests_passed=false
