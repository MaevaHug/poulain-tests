...
cc -Wall -Wextra -Werror -o aff_f aff_f.c
if [ $? -ne 0 ]; then
...
longest_test="$> ./aff_f \"zaz sent le poney\" | cat -e"
...
run_test() {
	...
	echo "$separator"
	echo "$> ./aff_f $args | cat -e"

	output=$(eval ./aff_f $args | cat -e)
...
# Test cases
run_test "\"abc\"" "f\$" || all_tests_passed=false
run_test "\"dubO a POIL\"" "f\$" || all_tests_passed=false
run_test "\"zaz sent le poney\"" "f\$" || all_tests_passed=false
run_test "" "f\$" || all_tests_passed=false
