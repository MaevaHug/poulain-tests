...
cc -Wall -Wextra -Werror -o aff_last_param aff_last_param.c
if [ $? -ne 0 ]; then
...
longest_test="$> ./aff_last_param \"zaz\" \"mange\" \"des\" \"chats\" | cat -e"
...
run_test() {
	...
	echo "$separator"
	echo "$> ./aff_last_param $args | cat -e"

	output=$(eval ./aff_last_param $args | cat -e)
...
# Test cases
run_test "\"zaz\" \"mange\" \"des\" \"chats\"" "chats\$" || all_tests_passed=false
run_test "\"j'aime le savon\"" "j'aime le savon\$" || all_tests_passed=false
